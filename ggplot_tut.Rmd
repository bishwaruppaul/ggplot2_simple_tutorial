...
# Bar plots (mean-SD) - prepare data

Bar plots which show mean and SD values are one of the most popular plots to be displayed in research articles, but making them in R is a little complicated.
...
# Bar plots (mean-SD) - Create plot

Mean values are used to create the bars with `geom_col` command, and SD values are used to create the error bars with `geom_errorbar` command.
...
.pull-right[
![barplot](./ggplot_tut_files/figure-html/ex12.svg)
]
